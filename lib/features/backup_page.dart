  ...
  Future<void> _restoreFromFile(File file) async {
    ...
    if (confirmed == true && mounted) {
      ...
      try {
        ...
        if (mounted) {
          // Invalidate repository providers to refresh in-memory state
          ref.invalidate(reportRepositoryProvider);
          ref.invalidate(patientRepositoryProvider);
          ref.invalidate(templateRepositoryProvider);
          ref.invalidate(masterDataRepositoryProvider);

          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text('Restored database from ${p.basename(file.path)} successfully.'),
              behavior: SnackBarBehavior.floating,
              backgroundColor: const Color(0xFF0F766E),
            ),
          );
              ...
              child: Column(
                ...
                children: [
                  SectionHeader(
                    ...
                    subtitle: 'Manage local Isar storage, generate JSON/binary snapshots, and perform disaster recovery',
                  ...
                  const SectionHeader(
                    ...
                    subtitle: 'Execute immediate backups or import external JSON archives',
                  ),
                        ...
                        Expanded(
                          child: Column(
                            ...
                            children: [
                              ...
                              Text('Export full clinical database to JSON archive', style: context.textTheme.bodySmall?.copyWith(color: context.colorScheme.onSurfaceVariant)),
                            ],
                          ),
                        ),
                      ],
